...
/*   Updated: 2017/05/21 19:38:30 by qho              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_try_place(t_map *m, t_piece p, int r, int c) // return -1 when no go, if it can be placed, return the score of position.
{
	...
	p_flag = 0;
	pr = -1;
	fprintf(stderr, "%s testing piece at r: %d and c %d %s\n", R, r, c, W);

	// fprintf(stderr, "piece shape: \n");
	// fprintf(stderr, "map shape: \n");
	while (++pr < p.height && r < (m->height - p.true_h - p.r + 1))
	{
		pc = -1;
		mc = c;
		while (++pc < p.width && mc < (m->width - p.true_w - p.c + 1))
		{
			// fprintf(stderr, "%d", p.shape[pr][pc]);
			if (r >= 0 && mc >= 0)
			{
				// fprintf(stderr, "%d", m->map[r][mc]);
